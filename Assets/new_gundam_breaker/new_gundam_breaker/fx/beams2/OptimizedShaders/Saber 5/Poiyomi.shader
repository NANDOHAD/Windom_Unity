Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/b3690da4cc758b0408002ab8d13f3420"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP2 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1) == 1) // Basic
				{
					...
					edgeAlpha = remapClamped(da + float(0.025), da, noise) * (1 - dissolveAlpha);
				}
				else if (float(1) == 2) // Point to Point
				{
					...
					float3 currentPos;
					float distanceTo = 0;
					direction = normalize(float4(0,1,0,0) - float4(0,0,0,0));
					currentPos = lerp(float4(0,0,0,0), float4(0,1,0,0), dissolveAlpha);
				...
				if (float(0.025))
				{
					...
					#endif
					poiFragData.baseColor = lerp(poiFragData.baseColor, edgeColor.rgb, smoothstep(0, 1 - float(0.5) * .99999999999, edgeAlpha));
				}
				poiFragData.emission += lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(0), smoothstep(0, 1 - float(0.5) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(3.81);
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.5333334,0.6449554,1,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.5333334,0.6449554,1,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.5333334,0.6449554,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = float(7);
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.4276319,1,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.4276319,1,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.4276319,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			float3 applyEmission2(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength2 = float(9.59);
				float glowInTheDarkMultiplier2 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP2) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor2 = POI2D_SAMPLER_PAN(_EmissionMap2, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.5333334,0.6483796,1,1).rgb, float(0));
				}
				else
				{
					emissionColor2 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap2, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.5333334,0.6483796,1,1).rgb, float(0));
				}
				#else
				emissionColor2 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.5333334,0.6483796,1,1).rgb, float(0));
				#endif
				if (float(1))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE2) || !defined(OPTIMIZER_ENABLED)
						emissionStrength2 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve2, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos2, float4(0,20,0,0)) * float(4)) + _Time.x * float(15));
						#endif
					}
					else
					{
						emissionStrength2 *= calculateScrollingEmission(float4(0,20,0,0), float(15), float(4), float(2), float(5), pos2);
					}
				}
				if (float(0))
				{
