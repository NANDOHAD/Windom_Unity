Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/035b33d8f052b794380f71b96b19277c"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP2 
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = float(6.71);
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.1876442,1,0,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.1876442,1,0,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.1876442,1,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			float3 applyEmission2(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength2 = float(13.51);
				float glowInTheDarkMultiplier2 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP2) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor2 = POI2D_SAMPLER_PAN(_EmissionMap2, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
					emissionColor2 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap2, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				#else
				emissionColor2 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				if (float(1))
				{
					emissionStrength2 *= calculateBlinkingEmission(float(0.5), float(1), float(4), float(0));
				}
